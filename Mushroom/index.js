...
import {prefix, getVersion, RightClick, radians_to_degrees} from "./Code/Utils"
...
class buttondraw {
    ...
    clickedbutton() {
      ...
      let buttonx = this.x + (this.width / 2 - Renderer.getStringWidth(this.text) / 2.3);
      let buttony = this.y + (this.height / 2 - 4);
      Renderer.drawRect(Renderer.color(20, 150, 100, 250), this.x, this.y, this.width * 1.1, this.height);
  ...
  titlebutton() {
    ...
    let buttony = this.y + (this.height / 2 - 4);
      Renderer.drawRect(Renderer.color(0, 0, 0, 200), this.x, this.y, this.width * 1.1, this.height);
    Renderer.drawString(buttontext, buttonx, buttony);
}
    ...
    drawButton() {
        ...
        if (mx >= this.x && mx <= this.x + this.width && my >= this.y && my <= this.y + this.height) {
            Renderer.drawRect(Renderer.color(20, 150, 100, 150), this.x, this.y, this.width * 1.1, this.height);
        } else {
            Renderer.drawRect(Renderer.color(50, 50, 50, 200), this.x, this.y, this.width * 1.1, this.height);
        }
        Renderer.drawString(buttontext, buttonx, buttony);
    }
    ...
    extrabutton() {
      ...
      let mx = Client.getMouseX();
      let my = Client.getMouseY();
      Renderer.drawRect(Renderer.color(25, 25, 25, 250), this.x, this.y, this.width * 1.1, this.height);
      Renderer.drawRect(Renderer.color(250, 250, 250, 250), this.x + 91.3, this.y + 1, this.width / 12, this.height / 1.25);
      if (mx >= this.x && mx <= this.x + this.width && my >= this.y && my <= this.y + this.height) {
        Renderer.drawRect(Renderer.color(0, 0, 250, 150), this.x + 91.8, this.y + 1.3, this.width / 13.5, this.height / 1.35);
      }
      Renderer.drawString(buttontext, buttonx, buttony);
  }
  extrabuttontoggled() {
    ...
    let buttonx = this.x + (this.width / 2 - Renderer.getStringWidth(this.text) / 2);
    let buttony = this.y + (this.height / 2 - 4);
    Renderer.drawRect(Renderer.color(0, 0, 250, 250), this.x + 91.8, this.y + 1.3, this.width / 13.5, this.height / 1.35);
    Renderer.drawString(buttontext, buttonx, buttony);
}
sliderdraw() {
  let buttontext = this.text;
...
keybindbutton() {
  ...
  let buttony = this.y + (this.height / 2 - 4);
  Renderer.drawRect(Renderer.color(25, 25,25, 250), this.x, this.y, this.width * 1.1 , this.height);
  Renderer.drawString("Keybind", this.x + 28, this.y + 1)
  Renderer.drawString("[  ]", this.x + 90, this.y + 1)
  Renderer.drawString(buttontext, buttonx, buttony);
}
commingsoontm() {
  let buttontext = this.text;
...
register("renderOverlay", function() {
    if (configGui.isOpen()) {
        let mushroomer = new Text("&fMushroom Client", Renderer.screen.getWidth() / 2 - 90 - Renderer.getStringWidth(mushroomer) * 1.1, Renderer.screen.getHeight() / 1.4).setScale(3).setShadow(true);
        let mushroomersmall = new Text("&cGet Ready Hypixel!", Renderer.screen.getWidth() / 2 - 90 - Renderer.getStringWidth(mushroomersmall) * 1.1, Renderer.screen.getHeight() / 1.3).setScale(1.5).setShadow(true);
        mushroomer.draw();
        mushroomersmall.draw();
    }
})  
...
//moveable mco gui
register("step", () => {
  if (!letmovecombatgui) return;
  Config.combatxpog = Client.getMouseX() - 50
  Config.combatypog = Client.getMouseY() - 5
});
...
spanishbutton = new buttondraw(otherx,spanishy,100,10, "Spanish Translator");
...
freezebutton = new buttondraw(otherx,freezey,100,10, "Freeze (Use Caution)");
...
const Menuopen = new KeyBind("Open Menu", Keyboard.KEY_NONE, "Mushroom");
register("step", () => {

  if (Client.currentGui.get() == null) {
    if (Menuopen.isPressed()) {
      ChatLib.command("mc", true)
    }
  }
const ClickMenuOpen = new KeyBind("Open Click Menu", Keyboard.KEY_NONE, "Mushroom");
if (Client.currentGui.get() == null) {
  if (ClickMenuOpen.isPressed()) {
    ChatLib.command("mco", true)
  }
}
})
